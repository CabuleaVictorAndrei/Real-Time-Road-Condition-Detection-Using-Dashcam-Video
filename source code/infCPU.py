...
frame_queue = queue.Queue(maxsize=5)  # buffer between CPU and inference
...
def cpu_worker():
    ...
    while True:
        ...
        cpu_fps = 1.0 / inference_time if inference_time > 0 else 0
        ...
        print(f"Frame {frame_counter}: Pred={predicted_class}, Conf={confidence:.4f}, "
              ...
              f"Inference={inference_time * 1000:.2f} ms ({cpu_fps:.2f} FPS), "
              f"Real FPS={real_fps:.2f}, "
              )

    avg_cpu_fps = frame_counter / total_inference_time
    avg_real_fps = frame_counter / total_loop_time
    print(f"\nProcessed {frame_counter} frames.")
    print(f"Average CPU FPS (inference only): {avg_cpu_fps:.2f}")
